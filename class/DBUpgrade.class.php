...
class DBUpgrade {
	...
	public static function AddSetting($id, $setting, $defvalue){
        if(Database::getSetting($setting) === NULL){
			...
			return $stmt->execute() or die($stmt->errorinfo()[2]."\n");
	  }
	  return TRUE;
	}
	...
	public static function AddColumn($table, $column, $column_definition){
        if(!Database::columnExists($table, $column))
			return Database::addNewColumn($table, $column, $column_definition);
	    return TRUE;
	}
	...
	public static function Upgrade() {
        ...
        if($version < 1)
            $OperationResult = 
                ...
                self::AddColumn('torrent', 'hash', 'VARCHAR(40) NOT NULL'); //If a column already exists it just does nothing
