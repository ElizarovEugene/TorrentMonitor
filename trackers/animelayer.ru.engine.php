...
class animelayerru
{
        ...
        if (Database::checkTrackersCredentialsExist($tracker))
        {
            ...
            if ( ! empty($page))
            {
                ...
                if (preg_match('/Имя пользователя или пароль неверны/', $page, $array))
                {
                    ...
                    animelayerru::$exucution = FALSE;
                }
                // Если подходят - получаем куки
                elseif (preg_match_all('/Set-Cookie: (.+);/iU', $page, $array))
                {
                    animelayerru::$sess_cookie = '';
                    foreach ($array[1] as $val)
                        animelayerru::$sess_cookie .= $val.'; ';
                    Database::setCookie($tracker, animelayerru::$sess_cookie);
                    // Запускам процесс выполнения
                    animelayerru::$exucution = TRUE;
                }
                else
                {
                    // Устанавливаем варнинг
                    if (animelayerru::$warning == NULL)
                    {
                        animelayerru::$warning = TRUE;
    ...
    public static function main($torrentInfo)
    {
        ...
        if (animelayerru::checkCookie($cookie))
        {
            animelayerru::$sess_cookie = $cookie;
            // Запускам процесс выполнения
            animelayerru::$exucution = TRUE;
        }           
        else
            animelayerru::getCookie($tracker);
        
        if (animelayerru::$exucution)
        {
            ...
            $page = Sys::getUrlContent(
                array(
                    ...
                    'cookie'         => animelayerru::$sess_cookie,
                    'sendHeader'     => array('Host' => 'animelayer.ru', 'Content-length' => strlen(animelayerru::$sess_cookie)),
                    'convert'        => array('windows-1251', 'utf-8//IGNORE'),
                )
            );
                ...
                if (preg_match('/<td width=\"\" class=\"heading\" valign=\"top\" align=\"right\">Добавлен<\/td><td valign=\"top\" align=\"left\">(.*)<\/td>/', $page, $array))
                {
                    ...
                    if (isset($array[1]))
                    {
                        if ( ! empty($array[1]))
                        {
                            ...
                            $date = $array[1];
                            $date_str = animelayerru::dateNumToString($array[1]);
