...
class casstudiotv
{
		...
		if (Database::checkTrackersCredentialsExist($tracker))
		{
			...
			if ( ! empty($page))
			{
				...
				if (preg_match('/Вы ввели неверное имя пользователя. Проверьте его и попробуйте ввести ещё раз./', $page, $array))
				{
					...
					casstudiotv::$exucution = FALSE;
				}
				//если подходят - получаем куки
				elseif (preg_match_all('/Set-Cookie: (.*);/iU', $page, $array))
				{
					casstudiotv::$sess_cookie = $array[1][3].'; '.$array[1][4].'; '.$array[1][5];
					Database::setCookie($tracker, casstudiotv::$sess_cookie);
					//запускам процесс выполнения, т.к. не может работать без кук
					casstudiotv::$exucution = TRUE;
				}
				else
				{
					//устанавливаем варнинг
					if (casstudiotv::$warning == NULL)
					{
						casstudiotv::$warning = TRUE;
	...
	public static function main($torrentInfo)
	{
		...
		if (casstudiotv::checkCookie($cookie))
		{
			casstudiotv::$sess_cookie = $cookie;
			//запускам процесс выполнения
			casstudiotv::$exucution = TRUE;
		}
		else
    		casstudiotv::getCookie($tracker);
		
		if (casstudiotv::$exucution)
		{
            ...
            $page = Sys::getUrlContent(
            	array(
            		...
            		'cookie'         => casstudiotv::$sess_cookie,
            		'sendHeader'     => array('Host' => 'casstudio.tv', 'Content-length' => strlen(casstudiotv::$sess_cookie)),
            	)
            );
				...
				if (preg_match('/<b>Добавлен<\/b>: <span class=\"my_tt\" title=\"(.*)\">(.*)<\/span>/', $page, $array))
				{
					...
					if (isset($array[2]))
					{
						...
						if ( ! empty($array[2]))
						{
                            ...
                            $date = casstudiotv::dateStringToNum($array[2]);
							$date_str = casstudiotv::dateNumToString($date);
    ...
    public static function generateURL($tracker, $torrent_id) {
        return 'http://'.$tracker.'/viewtopic.php?id='.$torrent_id;
    }
