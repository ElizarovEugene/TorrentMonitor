...
class kinozalguru
{
	...
	{
		$path = kinozalguru::sizesFilePath();
		if ( ! file_exists($path))
			return array();
	...
	public static function work($id, $tracker, $name, $torrent_id, $hash, &$return)
	{
		...
		else
		{
            if (Sys::checkTorrentFile($torrent))
            {
            	...
            	$creationDate = kinozalguru::torrentCreationDate($torrent);
            	$timestamp    = $creationDate !== NULL ? $creationDate : time();
            	$date         = date('Y-m-d H:i:s', $timestamp);
            	$date_str     = kinozalguru::dateNumToString($timestamp);
				...
				if ($saved)
				{
				    ...
				    kinozalguru::writeSize($tracker, $torrent_id, strlen($torrent));
				}
				else
	...
	public static function fetchName($torrent_id)
	{
		...
		return kinozalguru::torrentInfoName($torrent);
	}
	...
	public static function getRequestParams($params)
	{
		...
		$options = array(
			...
			CURLOPT_REFERER        => 'https://kinozal.guru/',
		);
	...
	public static function parse($params, $page)
	{
		...
		if ($code == 200 && stripos($contentType, 'application/x-bittorrent') !== FALSE)
		{
			...
			if ($isNewTopic)
				...
				kinozalguru::work($id, $tracker, $name, $torrent_id, $hash, $return);
			elseif ($storedSize === NULL)
			{
				...
				if ($contentLength !== NULL)
					kinozalguru::writeSize($tracker, $torrent_id, $contentLength);
				$return[$id]['error'] = 0;
			}
