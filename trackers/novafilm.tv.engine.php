...
class novafilmtv
{
        ...
        if (preg_match('/'.$name.'/i', $item->title))
        {
            if ($hd == 1)
            {
                if (preg_match_all('/720/', $item->link, $matches))
                {
                    ...
                    {
                        $episode = $matches[0];
                        $date = novafilmtv::dateStringToNum($item->pubDate);
    ...
    {   
        ...
        if (Database::checkTrackersCredentialsExist($tracker))
        {
            ...
            if ( ! empty($page))
            {
                ...
                {
                    novafilmtv::$sess_cookie = $array[1][2].'='.$array[2][2];
                    Database::setCookie($tracker, novafilmtv::$sess_cookie);
                    //запускам процесс выполнения, т.к. не может работать без кук
                    novafilmtv::$exucution = TRUE;
                }
                //проверяем нет ли сообщения о неправильном логине/пароле
                elseif (preg_match('/\/do\/recover/', $page, $out))
                {
                    //устанавливаем варнинг
                    if (novafilmtv::$warning == NULL)
                    {
                        novafilmtv::$warning = TRUE;
                        Errors::setWarnings($tracker, 'credential_wrong');
                    }
                    //останавливаем выполнение цепочки
                    novafilmtv::$exucution = FALSE;
                }
    ...
    public static function main($torrentInfo)
    {
        ...
        if (empty(novafilmtv::$exucution) || (novafilmtv::$exucution))
        {
            //проверяем получена ли уже кука
            if (empty(novafilmtv::$sess_cookie))
            {
                $cookie = Database::getCookie($tracker);
                if (novafilmtv::checkCookie($cookie))
                {
                    novafilmtv::$sess_cookie = $cookie;
                    //запускам процесс выполнения
                    novafilmtv::$exucution = TRUE;
                }
                else
                    novafilmtv::getCookie($tracker);
            }
            
            //проверяем получена ли уже RSS лента
            if ( ! novafilmtv::$log_page)
            {
                if (novafilmtv::$exucution)
                {
                    //получаем страницу
                    novafilmtv::$page = Sys::getUrlContent(
                        array(
                            'type'           => 'GET',
                            'returntransfer' => 1,
                    ...
                    if ( ! empty(novafilmtv::$page))
                    {
                        //читаем xml
                        novafilmtv::$xml_page = @simplexml_load_string(novafilmtv::$page);
                        //если XML пришёл с ошибками - останавливаем выполнение, иначе - ставим флажок, что получаем страницу
                        if ( ! novafilmtv::$xml_page)
                        {
                            //устанавливаем варнинг
                            if (novafilmtv::$warning == NULL)
                            {
        ...
        if (novafilmtv::$exucution)
        {
            if ( ! empty(novafilmtv::$xml_page))
            {
                //сбрасываем варнинг
                Database::clearWarnings($tracker);
                $nodes = array();
                foreach (novafilmtv::$xml_page->channel->item AS $item)
                {
                ...
                {
                    $serial = novafilmtv::analysis($name, $hd, $item);
                    if ( ! empty($serial))
                    {
                        $episode = substr($serial['episode'], 4, 2);
                        $season = substr($serial['episode'], 1, 2);
                        $date_str = novafilmtv::dateNumToString($serial['date']);
                        ...
                        if ($download)
                        {
                            ...
                            $torrent = Sys::getUrlContent(
                                array(
                                    ...
                                    'returntransfer' => 1,
                                    'url'            => $serial['link'],
                                    'cookie'         => novafilmtv::$sess_cookie,
                                    'sendHeader'     => array('Host' => 'novafilm.tv', 'Content-length' => strlen(novafilmtv::$sess_cookie)),
                                )
                            );
