        ...
        if (Database::checkTrackersCredentialsExist($tracker))
        {
            ...
            if ( ! empty($page))
            {
                ...
                if (preg_match('/Вы ввели неверное\/неактивное имя пользователя или неверный пароль/', $page, $array))
                {
                    ...
                    pornolabnet::$exucution = FALSE;
                }
                //если подходят - получаем куки
                elseif (preg_match('/bb_data=.+;/U', $page, $array))
                {
                    pornolabnet::$sess_cookie = $array[0];
                    Database::setCookie($tracker, pornolabnet::$sess_cookie);
                ...
                {
                    //устанавливаем варнинг
                    if (pornolabnet::$warning == NULL)
                    {
                        pornolabnet::$warning = TRUE;
        ...
        if (pornolabnet::$exucution)
        {
            ...
            $page = Sys::getUrlContent(
                array(
                    ...
                    'sendHeader'     => array('Host' => 'pornolab.net', 'Content-length' => strlen(pornolabnet::$sess_cookie)),
                    'convert'        => array('windows-1251', 'utf-8//IGNORE'),
                )
            );
                ...
                if (preg_match('/<span title=\"Зарегистрирован\">\[ (.+) \]<\/span>/', $page, $array))
                {
                    ...
                    if (isset($array[1]))
                    {
                        ...
                        if ( ! empty($array[1]))
                        {
                            ...
                            $date = pornolabnet::dateStringToNum($array[1]);
                            $date_str = pornolabnet::dateNumToString($array[1]);
